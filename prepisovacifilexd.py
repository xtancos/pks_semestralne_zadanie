...
def create_header(msg_type: int, flags: int, length: int, msg_id: int, total_fragments: int, current_fragment: int, data: bytes) -> bytes:
    ...
    """
    # Prvý bajt: msg_type (4 bity) a flags (4 bity)
    first_byte = (msg_type << 4) | flags

    # Predbežná hlavička bez CRC (nastavíme CRC ako 0 na začiatok)
    header_format = "!B H B B B H"  # B = 1 byte, H = 2 bytes
    temp_header = struct.pack(header_format, first_byte, length, msg_id, total_fragments, current_fragment, 0)


    # Spočítajte CRC pre hlavičku + dáta
    crc = crc16.crc16xmodem(temp_header + data)

    # Vytvorte finálnu hlavičku s CRC
...
def listener():
    ...
    while not end_connection:
        try:
            ...
            computed_crc = crc16.crc16xmodem(header + body)
            # msg_id = header_info['msg_id']
    ...
    while not end_connection:
        try:
            ...
            if message[:4] == "/max":
                ...
                print(f"Fragment size sent to {max_fragment_size}")
                continue
...
def send_message(message, max_fragment_size=100):
    ...
    print(total_fragments , "toto je max size of fragmets")
    ...
    for current_fragment, fragment_data in enumerate(fragments, start=1):
        ...
        header = create_header(msg_type, flags, length, msg_id, total_fragments, current_fragment, fragment_data.encode())

        # Odoslanie fragmentu
        udp_socket.sendto(header + fragment_data.encode(), (REMOTE_IP, REMOTE_PORT))
